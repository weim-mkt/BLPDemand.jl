...
Compute market shares in BLP random coefficients demand model for each market given δ.
...
# Arguments:
- `δ` vector of length `J=` number of products: mean utility of product j
...
See also: [`delta`](@ref)
"""
function share(δ::AbstractVector, # J * 1; mean utility
  σ::AbstractVector, # K * 1; standard deviation of random coefficients
  x::AbstractMatrix, # J * K; product characteristics
  ν::AbstractMatrix) # K * S; random draws
  J = length(δ)
  K = length(σ)
  S = size(ν, 2) # number of simulation draws
  ...
  s ./= S
  s .+= eps(zero(eltype(s))) # avoid very small values
  #@show s, δ
  return (s)
end
...
Compute market shares in BLP random coefficients demand model for each market given parameters.
